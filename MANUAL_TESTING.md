...
- [ ] `/candidate-testnet` in the general-chat channel: grants `Testnet Validator Candidate` and replies ephemerally with the exact "Reply to someone asking to become a validator" wording from `Shared.md` (no DM). No Sheet row is created at intake; the candidate's row is created later by `/submit-request`.
...
- [ ] `Decline` (right-click the notification in `#validator-review`): the modal collects a multi-line criteria list and shows the "no confidential info" placeholder warning; submitting it keeps the `Testnet Validator Candidate` role, posts the exact decline wording in `#testnet-onboarding` (pinging the candidate, criteria as bullets), and updates the Sheet row's `Status` (`Declined`), `Missing criteria`, `Decision date`, `Reviewers`.
- [ ] `Approve`: no modal; sets the Sheet row's `Status` to `GovDAO pending`, posts the exact "Approve a candidate" wording in `#testnet-onboarding` (pinging the candidate), and posts a message in `#validator-review` tagging the configured GovDAO contact with the candidate's Valoper link. It does NOT grant `Testnet Validator` or remove the Candidate role; the activation poller does that once the validator joins the active set.
...
- [ ] Resubmission: after a `Declined` row, the candidate re-runs `/submit-request` (decline keeps the Candidate role, so no `/candidate-testnet` needed); confirm a brand-new Sheet row is appended (the first row is left untouched) and a new notification references the new row.
- [ ] Welcome/ack delivery: `/candidate-testnet` and `/submit-request` reply ephemerally (only the candidate who ran them sees it); no DM and no channel post.
- [ ] Activation: a `GovDAO pending` row whose validator is in the active set → the poller writes `GovDAO approved`, grants `Testnet Validator` (removing Candidate), and posts the `activated` wording in `#testnet-onboarding` pinging the candidate (no DM). Confirm `#testnet-onboarding` is readable by both the Candidate and Validator roles.
- [ ] Decline delivery + approve visibility: run `Decline` against a candidate and confirm the `Testnet Validator Candidate` role is kept, the decline post appears in `#testnet-onboarding` pinging the candidate, and they can re-apply with `/submit-request`. Run `Approve` against another candidate and confirm the approval post appears in `#testnet-onboarding` pinging the candidate.
