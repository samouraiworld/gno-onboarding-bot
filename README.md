...
- `Approve`, `Decline` — reviewer message-context decisions; right-click the submission notification in `#validator-review`. Both notify the candidate in `#testnet-onboarding`; Decline keeps the candidate role so the candidate can reapply with `/submit-request`
...
3. **Installation** tab → under *Default Install Settings*:
   ...
   - Permissions (integer **268520448**):
     ...
     | Send Messages | post in `#validator-review` and `#testnet-onboarding` (candidate notifications) |
...
8. **Candidate-facing channels** — the bot never DMs. Candidate-run commands (`/candidate-testnet`, `/submit-request`) reply ephemerally. Bot-initiated notices ping the candidate via a channel post: approval, decline, and the activation notice all go to `onboarding_channel_id` (must be readable by **both** the Candidate and Validator roles, since the candidate holds Candidate at approve/decline time and Validator at activation). The bot needs Send Messages in the onboarding channel.
