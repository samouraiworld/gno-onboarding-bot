...
- **The bot never DMs.** Candidate-run commands reply ephemerally (welcome on `/candidate-testnet`, acknowledgement on `/submit-request`). Bot-initiated notices post to a channel and ping the candidate (`sendCandidateMessage` in handlers, `notifyCandidate` in the poller): approval, decline, and the activation notice all go to the onboarding channel (readable by both the Candidate and Validator roles, since the candidate holds Candidate at approve/decline and Validator at activation). DMs to non-mutual or DM-disabled users are unreliable and can get the bot flagged.
- **Decline keeps the candidate role** and posts the reasons to the onboarding channel, the same channel approve uses. The candidate retains onboarding access and reapplies by running `/submit-request` again. The decline modal warns the reviewer to keep the text free of confidential info, since the channel is public.
...
- **`/harvest` omits already-imported candidates from the `harvest.json` attachment** (Readiness col P populated by a prior `/harvest-import`, excluding the `Duplicate of row N` marker — see `isImported`), so each file carries only candidates still awaiting a digest. Those rows stay in the sheet writes: their Y/Z and the Evidence tab still refresh every run (the tab is cleared and rewritten from all active candidates).
...
The switch from candidate DMs to channel posts (onboarding channel for welcome, acknowledgement, approval, and decline), plus removing the `Request missing info` and `Escalate to call` commands, is specced under `.claude/docs/superpowers/specs/2026-06-23-remove-dm-channel-post-design.md`.
