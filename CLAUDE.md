...
- `internal/sheet` — the Sheet schema (13 intake columns A-M, plus harvest assessment columns N-Y incl. the seven criterion checkboxes P-V), `Ensure`/`EnsureApprovedView`/`EnsureHarvestLayout` provisioning, `API` interface (fake-tested), and the real `google.golang.org/api/sheets/v4` client adapter.
...
- **Candidate-facing messages post to a channel and ping the candidate** (`sendCandidateMessage`), never by DM. DMs to non-mutual or DM-disabled users are unreliable and can get the bot flagged. Welcome, acknowledgement, and approval go to the onboarding channel, which must be readable by both the Candidate and Validator roles since Approve posts after swapping the candidate to Validator. Decline goes to general chat instead: it removes the candidate role, and the now-roleless candidate keeps access to general but not to the onboarding channel.
- **Decline removes the candidate role**, then posts the reasons to general chat. The decline modal warns the reviewer to keep the text free of confidential info, since general is public. A candidate reapplies from scratch with `/candidate-testnet`.
- **Channel-post fallback**: if a candidate-triggered command's post fails, fall back to an ephemeral reply with the same real message content (not a generic error). If a reviewer-triggered command's post fails, tell the reviewer so they can relay it manually, still including the real message text.
...
- **The harvest writes only its assessment columns (N-Y), never the human cells (A-M).** `/harvest` refreshes Red flags (W) and Engagement (X); `/harvest-import` refreshes Readiness (N), Summary (O), the seven criterion checkboxes (P-V), and Evidence links (Y). Assessment columns refresh each run. Curating good validators is done via the Status column + PR #4's `-approved` view, not a Selected column.
...
The switch from candidate DMs to channel posts (onboarding channel for welcome, acknowledgement, and approval; general chat for decline), plus removing the `Request missing info` and `Escalate to call` commands, is specced under `.claude/docs/superpowers/specs/2026-06-23-remove-dm-channel-post-design.md`.
