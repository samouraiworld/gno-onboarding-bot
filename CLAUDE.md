...
- `internal/sheet` — the Sheet schema (13 intake columns A-M, plus harvest assessment columns N-Z incl. the seven criterion checkboxes P-V and the human `Selected` checkbox Z), `Ensure`/`EnsureApprovedView`/`EnsureHarvestLayout` provisioning, `API` interface (fake-tested), and the real `google.golang.org/api/sheets/v4` client adapter.
...
- **The harvest writes only its assessment columns (N-Z), never the human cells.** `/harvest` refreshes Red flags (W) and Engagement (X); `/harvest-import` refreshes Readiness (N), Summary (O), the seven criterion checkboxes (P-V), and Evidence links (Y). The bot must never write A-M or the `Selected` checkbox (Z). Assessment columns refresh each run; human curation lives in `Selected`, which the bot only creates as a checkbox.
